    ...
    <OnboardingLayout
      ...
      footer={
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={handleNext} disabled={status === "testing"} className="rounded-full px-8 shadow-md">
            {t("common.next", "Next")} →
          </Button>
        </div>
        </>
    ...
    >
      <div className="animate-in fade-in slide-in-from-right-4 duration-500 flex-1 flex flex-col justify-center">
        <div className="space-y-2 text-center mb-6">
          <h2 className="text-2xl font-bold tracking-tight">{t("onboarding.translation.title", "Translation Engine")}</h2>
          <p className="text-sm text-muted-foreground">{t("onboarding.translation.desc", "Enable seamless bilingual reading with your preferred engine.")}</p>
        </div>

        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">{t("settings.translationProvider", "Provider")}</label>
            <Select value={provider} onValueChange={(v: "ai" | "deepl") => { setProvider(v); setStatus("idle"); }}>
          ...
          {provider === "deepl" && (
            <div className="space-y-1.5 animate-in fade-in zoom-in-95">
              <label className="text-xs font-medium text-muted-foreground">{t("settings.apiKey", "DeepL API Key")}</label>
              <Input 
                type="password" 
                value={apiKey}
        ...
        {provider === "deepl" && (
          <div className="mt-4 flex items-center gap-4">
            <Button variant="outline" size="sm" onClick={testConnection} disabled={status === "testing" || !apiKey}>
