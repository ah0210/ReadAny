  ...
  return (
    <OnboardingLayout
      ...
      footer={
        ...
        <Button onClick={onNext} size="lg" className="rounded-full px-8 shadow-lg shadow-primary/25 group">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 flex-1 flex flex-col justify-center">
        <div className="space-y-2 text-center mb-6">
          <h1 className="text-3xl font-extrabold tracking-tight">
            {t("onboarding.welcome.title", "Welcome to ReadAny").split(" ").slice(0, 2).join(" ")} <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-500">{t("onboarding.welcome.title", "Welcome to ReadAny").split(" ").slice(2).join(" ")}</span>
          </h1>
          <p className="text-sm text-muted-foreground">
            {t("onboarding.welcome.subtitle", "Your intelligent reading companion. Let's set up your ultimate reading environment.")}
          </p>
        </div>

        <div className="grid grid-cols-3 gap-4">
          {features.map((f, i) => (
            <div key={i} className="flex flex-col items-center gap-2 rounded-xl bg-muted/40 p-4 text-center transition-colors hover:bg-muted/80">
